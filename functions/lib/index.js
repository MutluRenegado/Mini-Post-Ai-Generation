...
exports.apiAiChain = exports.processScheduledPosts = exports.apiSocialWebhook = exports.apiSocialReply = exports.apiSocialInbox = exports.apiGeneratePost = exports.apiPostsPublish = exports.apiSocialConnect = exports.apiAuthOnSignup = void 0;
 ...
 */
exports.apiAuthOnSignup = (0, https_1.onRequest)({ cors: true, secrets: secrets_1.allSecrets }, async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).json({ error: 'Method Not Allowed' });
        return;
    }
    const decoded = await (0, auth_1.verifyAuthToken)(req, res);
    if (!decoded)
        return;
    try {
        ...
        const apiKey = secrets_1.postproxyApiKeySecret.value() || process.env.POST_PROXY_MEGA_API_KEY || '';
        ...
        await (0, firestore_1.getFirestore)().doc(`users/${uid}`).set({
            userId: uid,
            userEmail: email,
            profileGroupId,
 ...
 */
exports.apiSocialConnect = (0, https_1.onRequest)({ cors: true, secrets: secrets_1.allSecrets }, async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).json({ error: 'Method Not Allowed' });
        return;
    }
 ...
 */
exports.apiPostsPublish = (0, https_1.onRequest)({ cors: true, secrets: secrets_1.allSecrets }, async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).json({ error: 'Method Not Allowed' });
        return;
    }
    ...
    try {
        ...
        const quotaResult = await (0, quota_1.enforceDailyQuota)(uid, tier);
        ...
        if (scheduledAt) {
            ...
            await (0, firestore_1.getFirestore)().collection('scheduled_posts').add({
                content,
                text: content,
                platforms,
                ...
                userId: uid,
                status: 'pending',
                scheduledAt: firestore_1.Timestamp.fromDate(scheduledTimeDate),
 ...
 */
exports.apiGeneratePost = (0, https_1.onRequest)({ cors: true, secrets: secrets_1.allSecrets }, async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).json({ error: 'Method Not Allowed' });
        return;
    }
    ...
    try {
        ...
        const apiKey = secrets_1.googleApiKeySecret.value() ||
            secrets_1.geminiApiKey2Secret.value() ||
            secrets_1.geminiApiKeySecret.value() ||
 ...
 */
exports.apiSocialInbox = (0, https_1.onRequest)({ cors: true, secrets: secrets_1.allSecrets }, async (req, res) => {
    if (req.method !== 'GET') {
        res.status(405).json({ error: 'Method Not Allowed' });
        return;
    }
    ...
    if (!decoded)
        return;
    try {
        const profileGroupId = req.query?.profileGroupId || '';
        const apiKey = secrets_1.postproxyApiKeySecret.value() || process.env.POST_PROXY_MEGA_API_KEY || '';
        let interactions = [];
        if (apiKey && profileGroupId) {
            const response = await fetch(`${POSTPROXY_BASE_URL}/interactions?profileGroupId=${encodeURIComponent(profileGroupId)}`, {
                method: 'GET',
 ...
 */
exports.apiSocialReply = (0, https_1.onRequest)({ cors: true, secrets: secrets_1.allSecrets }, async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).json({ error: 'Method Not Allowed' });
        return;
    }
    ...
    try {
        ...
        const apiKey = secrets_1.postproxyApiKeySecret.value() || process.env.POST_PROXY_MEGA_API_KEY || '';
        let result = { success: true, id: `reply_${Date.now()}` };
        if (apiKey) {
 ...
 */
exports.apiSocialWebhook = (0, https_1.onRequest)({ cors: true, secrets: secrets_1.allSecrets }, async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).json({ error: 'Method Not Allowed' });
        return;
    }
    try {
        const secret = secrets_1.webhookSecret.value() || secrets_1.postproxyApiKeySecret.value() || process.env.POSTPROXY_WEBHOOK_SECRET || '';
        const signature = req.headers['x-postproxy-signature'] || '';
        if (secret && signature) {
        ...
        if (eventType === 'post.published' || eventType === 'post.failed') {
            ...
            if (postId) {
                await (0, firestore_1.getFirestore)().doc(`posts/${postId}`).set({
 ...
 */
exports.processScheduledPosts = (0, scheduler_1.onSchedule)({
    schedule: 'every 5 minutes',
    secrets: secrets_1.allSecrets,
    timeZone: 'UTC',
    retryCount: 3,
}, async (event) => {
 ...
 */
exports.apiAiChain = (0, https_1.onRequest)({ cors: true, secrets: secrets_1.allSecrets }, async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).json({ error: 'Method Not Allowed' });
        return;
    }
...
Return raw JSON only.`;
        ...
        try {
            ...
            if (textOutput.facebook && textOutput.facebook.length > 2000)
                textOutput.facebook = textOutput.facebook.slice(0, 1997) + '...';
            if (textOutput.instagram && textOutput.instagram.length > 1500)
                textOutput.instagram = textOutput.instagram.slice(0, 1497) + '...';
            if (textOutput.linkedin && textOutput.linkedin.length > 3000)
                textOutput.linkedin = textOutput.linkedin.slice(0, 2997) + '...';
            if (textOutput.twitter && textOutput.twitter.length > 280)
                textOutput.twitter = textOutput.twitter.slice(0, 277) + '...';
            if (textOutput.tiktok && textOutput.tiktok.length > 600)
                textOutput.tiktok = textOutput.tiktok.slice(0, 597) + '...';
            if (textOutput.threads && textOutput.threads.length > 500)
                textOutput.threads = textOutput.threads.slice(0, 497) + '...';
        }
        catch (err) {
...
Choosing the appropriate framework requires evaluating logistical capability, risk tolerance, and financial control. Mastering these principles ensures that global transactions remain predictable, compliant, and mutually profitable.`;
        ...
        try {
            const { text: optText } = await (0, ai_1.generateText)({
                model: google('gemini-2.5-flash'),
            ...
            if (optText.trim())
                enhancedImagePrompt = optText.trim();
        }
        catch (err) {
