      ...
      while (attempts < maxAttempts) {
        ...
        try {
          ...
          platformsContent = request.platforms.map((platform) => {
            ...
            const imagePrompt = ImagePromptBuilder.build(aiContext, platform);

            if (parsed.imagePrompt && QualityAuditor.audit({}, parsed.imagePrompt, platform, knowledgeBase).score > 70) {
              imagePrompt.assembled = parsed.imagePrompt;
            }
